...
class CyberAPI:
    ...
    def _extract_text(self, file_path: str, max_chars: int = 1500) -> str:
        """
            ...
            for root, dirs, files in os.walk(folder_path, followlinks=False):
                ...
                for filename in files:
                    ...
                    try:
                        ...
                        # Stream live file path to the UI — throttled to
                        # every 15 files so the UI thread doesn't choke.
                        file_counter = len(scored_files) + skipped
                        if file_counter % 15 == 0 and MAIN_WINDOW is not None:
                            safe_path = full_path.replace("\\", "\\\\").replace("'", "\\'")
                            try:
                                ...
                                pass  # best-effort, never crash the scan
    ...
    def _save_report(
        ...
        inside BASE_DIR/reports/ on the USB drive.
    ...
    def _analyze_with_local_ai(self, raw_data: str) -> str:
        ...
        payload = {
            ...
            "n_predict": 800,   # Enough for complete JSON incl. closing braces
            "temperature": 0.2,
            "stop": ["```", "\n\n\n"],
        }
        ...
        try:
            response = requests.post(
                AI_SERVER_URL,
                json=payload,
                timeout=300,  # 5 min — CPU inference at ~10 tok/s needs room
            )
...
def main():
    ...
    window = webview.create_window(
        ...
        url        = index_html if os.path.isfile(index_html) else "http://localhost:3000",
        js_api     = api,
        width      = 1400,
        height     = 900,
        min_size   = (1024, 700),
