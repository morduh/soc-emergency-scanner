...
function LoadingState({ currentFile }) {
  const [phase, setPhase] = useState(0);
  const phases = [
    'Initializing local AI engine...',
    'Enumerating files and computing suspicion scores...',
    'Analyzing suspect files with local AI...',
  ...
  return (
    <div className="glass-card rounded-2xl border border-slate-700 p-10 flex flex-col items-center gap-8 animate-fade-in-up">
      {/* Animated rings */}
      <div className="relative w-24 h-24 flex items-center justify-center">
        ...
        <div className="absolute inset-2 rounded-full border-2 border-red-500/50 animate-spin-slow" />
        <div className="absolute inset-4 rounded-full border border-red-600/70" />
        <Icons.Shield />
      </div>

      {/* Phase text */}
      <div className="text-center space-y-2">
        <p className="text-lg font-bold text-white">{phases[phase]}</p>
        <p className="text-xs text-slate-500 font-mono">Traversing file system and generating local AI forensic matrix...</p>
      </div>
      ...
      {/* Live file path — streamed in real-time by Python backend */}
      {currentFile && (
        <div className="w-full max-w-md flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-900/70 border border-slate-800">
          ...
          <p
            className="text-xs font-mono text-green-400/80 truncate leading-none tracking-tight"
            title={currentFile}
          >
            {currentFile}
          </p>
      ...
      <div className="flex gap-2">
        {[0, 1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="w-1.5 h-1.5 rounded-full bg-green-400 animate-data-ticker"
...
export default function App() {
  ...
  // Expose global bridge so Python's evaluate_js can push live file names
  useEffect(() => {
    window.updateScanningFile = (path) => {
      setCurrentFile(path);
    };
    return () => {
  ...
  const handleScan = async () => {
    ...
    setError(null);
    setScanResult(null);
    setCurrentFile('');  // reset live-file display for new scan
  ...
  return (
    <div className="min-h-screen bg-slate-950 cyber-grid flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-8 space-y-6">
        ...
        {/* Loading */}
        {loading && <LoadingState currentFile={currentFile} />}
